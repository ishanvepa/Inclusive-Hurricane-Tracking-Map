...
(async () => {
 ...
 // Populate dropdown with styled divs instead of <option>
  ...
  mapContainer.focus(); // initial focus
  
  mapContainer.addEventListener('keydown', (e) => {
    const step = 2.0; // degrees of latitude/longitude
    let [lon, lat] = chart.mapView.center;
  
    switch (e.key) {
      case 'ArrowUp':
        chart.mapView.moveCenter([lon, lat + 2], true);
        break;
      case 'ArrowDown':
        chart.mapView.moveCenter([lon, lat + 2], true);
        break;
      case 'ArrowLeft':
        chart.mapView.moveCenter([lon - 2, lat], true);
        break;
      case 'ArrowRight':
        chart.mapView.moveCenter([lon + 2, lat], true);
        break;
